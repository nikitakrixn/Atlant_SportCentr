...
using Atlant_Project.Forms.Sections;
using System.Data.SqlClient;

namespace Atlant_Project.Forms.UserInterface
{
    public partial class Sections_uc : UserControl
    {
        ...
        private DataTable Select()
        {
            ...
            string query = "Exec Section_Select";
        ...
        private void grid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                using(frmSelectSections select = new frmSelectSections())
                {
                    select.NameSection = grid.CurrentRow.Cells[1].Value.ToString();
                    select.label1.Text = select.label1.Text + grid.CurrentRow.Cells[1].Value.ToString();
                    select.label2.Text = select.label2.Text + grid.CurrentRow.Cells[2].Value.ToString();
                    new PopupEffect.transparentBg(this, select);
        ...
        {
            if (Settings.UserRole.guest || Settings.UserRole.manager_raspisania || Settings.UserRole.manager_trainer || Settings.UserRole.manager_clientov)
            {
            ...
            else
            {
                ...
                try
                {
                    string messege = string.Format("Удалить секцию {0}", grid.CurrentRow.Cells[1].Value.ToString());
                    if (MessageBox.Show(messege, "Подтверждение удаления",
                    MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        // Удалить выбранные строку
                        EntityFramework.Sections group = database.Sections
                        .Where(g => g.sections_id == id)
                        .FirstOrDefault();

                        database.Sections.Remove(group);
                    }
                    // Сохранить изменения
                    if (database.ChangeTracker.HasChanges())
                    {
                        ...
                        {
                            Bunifu.Snackbar.Show(this.FindForm(), "Секция успешно удалёна.", 3000, Snackbar.Views.SnackbarDesigner.MessageTypes.Success);
                        }
                        else
                        {
        ...
        {
            if (Settings.UserRole.guest || Settings.UserRole.manager_clientov)
            {
                Bunifu.Snackbar.Show(this.FindForm(), "Не достаточно прав для этого действия", 3000, Snackbar.Views.SnackbarDesigner.MessageTypes.Error);
            }
            else
            {
                new PopupEffect.transparentBg(this, new frmAddSections());
                PopulateDatatoDatagrid();
            }
        }
    }
